...
import { PixelFahy, ScatteredFahys } from "@/components/fahy/PixelFahy";
...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Live neighborhood pulse, eco-challenges, and rewards — guided by Fahy.",
      },
  ...
  return (
    <AppShell>
      <header className="px-5 pt-10 pb-6 bg-gradient-to-b from-peach/25 to-transparent relative">
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-xs font-medium text-forest/60 mt-1">{k("home.greeting")}</p>
          </div>
          <div className="relative w-16 h-16 bg-white rounded-2xl shadow-sm border border-peach/30 grid place-items-center">
            <PixelFahy mood="happy" size={56} />
        ...
        <div className="grid grid-cols-3 gap-3">
          <MetricTile
            ...
            value="24"
            status={k("home.metric.aqi_status")}
            tone="sage"
          />
          <MetricTile
            label={k("home.metric.crowd")}
            value={k("home.metric.crowd_value")}
            status={k("home.metric.crowd_status")}
            tone="peach"
          />
          <MetricTile
            label={k("home.metric.noise")}
            value="42dB"
            status={k("home.metric.noise_status")}
            tone="forest"
          />
        </div>
      </header>
      ...
      <section className="px-5 mb-8 relative">
        <PixelFahy mood="excited" size={44} className="absolute -top-2 -left-1 -rotate-12 z-10" />
        <div className="bg-forest text-white p-5 rounded-3xl relative overflow-hidden">
          <div className="relative z-10">
            ...
            <p className="text-sm leading-relaxed max-w-[230px]">{k("home.rec.body")}</p>
            <Link
              to="/ecosystem"
          ...
          <PixelFahy mood="listening" size={48} className="absolute right-3 bottom-2 opacity-90" />
        </div>
      </section>

      <section className="px-5 mb-8">
        <div className="flex justify-between items-end mb-4">
          <h2 className="font-display font-bold text-lg leading-none">{k("home.challenge")}</h2>
          <span className="text-[11px] font-semibold text-forest/40">{k("home.collected")}</span>
        </div>
      ...
      <section className="px-5 mb-6 relative">
        <PixelFahy mood="proud" size={40} className="absolute -top-1 right-1 rotate-6 z-10" />
        <div className="grid grid-cols-2 gap-3">
          ...
          >
            ...
            <p className="text-xs text-forest/60">{k("home.tile.peach_coins")}</p>
            <p className="text-xl font-display font-bold">{formatCoins("1,240")}</p>
          </Link>
          <Link
            to="/culture"
            className="bg-forest p-4 rounded-3xl flex flex-col justify-between active:scale-[0.98] transition-transform"
          >
            <div className="flex -space-x-2">
              <div className="w-6 h-6 rounded-full bg-sage ring-2 ring-forest" />
              <div className="w-6 h-6 rounded-full bg-peach ring-2 ring-forest" />
              <div className="w-6 h-6 rounded-full bg-fahy-yellow ring-2 ring-forest text-[8px] flex items-center justify-center font-bold text-forest">
                +3
              </div>
            </div>
            <div>
              ...
              <p className="text-sm text-white font-bold">{k("home.tile.badges", { n: 5 })}</p>
            </div>
          </Link>
        </div>
      </section>

      <div className="px-5 mb-6 flex items-center justify-around opacity-90">
        <PixelFahy mood="curious" size={36} className="-rotate-6" />
        <PixelFahy mood="caring" size={42} />
        <PixelFahy mood="sleepy" size={36} className="rotate-6" />
        <PixelFahy mood="sad" size={34} className="-rotate-3 opacity-70" />
      </div>
...
function MetricTile({
  ...
  const accent =
    tone === "sage" ? "text-sage-deep" : tone === "peach" ? "text-peach" : "text-forest";
  return (
    <div className="bg-white p-3 rounded-2xl border border-black/5 shadow-xs">
      ...
      <span className={`text-lg font-display font-bold ${accent}`}>{value}</span>
      <span className={`text-[9px] block ${accent}/70 opacity-80`}>{status}</span>
    </div>
  );
}
...
function FeedSpecies({
  ...
  return (
    <div className="flex-shrink-0 w-36 bg-white rounded-3xl p-3 border border-black/5 shadow-sm">
      <div className={`aspect-square ${bg} rounded-2xl mb-3 relative grid place-items-center`}>
